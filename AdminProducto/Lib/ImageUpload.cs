...
namespace AdminProducto.Lib
{
    public class ImageUpload
    {
        ...
        public ImageResult RenameUploadFile(HttpPostedFile file, string name, Int32 counter = 0)
        {
            name = name.Replace(" ", "").ToLower();
            string nameExt = file.ContentType.Replace("image/", "");

            // Se utiliza para gradar la imagen con su nombre original
            //var fileName = Path.GetFileName(file.FileName);

            string prepend = "item_";
            string finalFileName = prepend + ((counter).ToString()) + "_" + name;
            if (System.IO.File.Exists
                (HttpContext.Current.Request.MapPath(UploadPath + finalFileName)))
            {
                // Si la imagen existe sera eliminada
                eliminarImagen(finalFileName);
                //file exists => add country try again
                // Si la imagen existe se la cambia el numbre mediate un incrementador.
                return RenameUploadFile(file, name, ++counter);
            }
            //file doesn't exist, upload item but validate first
            return UploadFile(file, finalFileName + "." + nameExt);
        }

        private ImageResult UploadFile(HttpPostedFile file, string fileName)
        {
        ...
        {
            string imgPath = HttpContext.Current.Request.MapPath(UploadPath + img);
